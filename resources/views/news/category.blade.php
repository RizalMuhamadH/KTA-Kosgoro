...
@section('section')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('category-news.store')}}" action="POST" id="form_new_category">
                        @csrf 
    ...
    <div id="EditCategory" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="my-modal-title" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('category-news.update')}}" action="POST" enctype="multipart/form-data" id="form_edit_category">
                        @csrf
                        <div class="form-group">
                            <label> Name </label>
                        ...
                        <div class="laravel-input">
                            <input type="hidden" name="id" id="news_category_id_edit">
    ...
    <form action="{{ route('category-news.delete') }}" method="POST" id="form_delete_category"
        style="display: none">
        @csrf
    ...
    <script>
        $(document).ready(function(){
            var table_category = $("#table_category").DataTable({
                ...
                processing: true,
                ajax: {
                    url: "{{env('APP_URL')}}/category-news/datatables",
                    method: "GET",
                    dataType: "JSON",
                },
                columns: [
            ...
            $("#table_category").on('click','.btn_edit',function(){
                $.ajax({
                    url: "{{env('APP_URL')}}/category-news/detail/"+$(this).data('id'),
                    dataType: 'JSON',
                    method: 'GET',
                    success:function(data){
                        $("#name_edit").val(data.name);
                        $("#news_category_id_edit").val(data.id);
            ...
            $(".btn_save_edit").on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    .then((result) => {
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{env('APP_URL')}}/category-news/update",
                                method: 'POST',
                                dataType: 'JSON',
            ...
            $("#table_category").on('click', ".btn_delete", function() {
                ...
                form_url = "{{env('APP_URL')}}/category-news/delete";
                Swal.fire({
                        title: "Apakah anda yakin?",
